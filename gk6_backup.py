                ...
                for event in item["event"]:
                    if event.get("listen") == "test":
                        ...
                        for line in script_lines:
                            ...
                            for var in matches:
                                print(
                                    f"DEBUG: found chaining variable via pm.environment.set: {var}"
                                )
                    ...
                    if event.get("listen") == "test":
                        ...
                        for line in script_lines:
                            ...
                            if match_local:
                                ...
                                print(
                                    f"DEBUG: API '{api_name}' → local var '{local_var_name}' = {local_var_expr}"
                                )
                        ...
                        for line in script_lines:
                            ...
                            if match_env:
                                ...
                                print(
                                    f"DEBUG: API '{api_name}' → env var '{env_var_name}' ← {assignment_expr}"
                                )
                                ...
                                if assignment_expr in local_var_definitions:
                                    ...
                                    print(
                                        f"DEBUG: Backtrack for '{env_var_name}' → {backtrack_chain}"
                                    )
...
def convert_variables(text, chaining_vars, env_vars):
    def replace_var(match):
        ...
        if var_name in chaining_vars:
            print(
                f"DEBUG: Treating variable '{var_name}' as chaining → ${{{var_name}}}"
            )
            return f"${{{var_name}}}"
        elif var_name in env_vars:
            print(
                f"DEBUG: Treating variable '{var_name}' as env → ${{__ENV.{var_name}}}"
            )
            return f"${{__ENV.{var_name}}}"
        else:
            print(
                f"WARNING: Variable '{var_name}' not found → fallback to env → ${{__ENV.{var_name}}}"
            )
            return f"${{__ENV.{var_name}}}"
...
def convert(
    ...
    """
    Converts a Postman collection into a k6 load testing script.
    """
    ...
    # Load and write environment variables
    if env:
        try:
    ...
    if all:
        ...
        try:
            indices = [
                int(i.strip()) - 1 for i in (include or "").split(",") if i.strip()
            ]
